...
class Dataset3D(BaseDataset):
    ...
    def __init__(self, cfg, set="train", analysisMode=False, **kwargs):
        ...
        if self.cameras_to_use != None:
            all_camera_names = [
                cam for cam in list(self.dataset["calibrations"].values())[0]
            ]
            camera_names = [
                cam for cam in list(self.reproTools.values())[0].cameras
            ]
            self.use_idxs = [
                i
                for i, cam in enumerate(all_camera_names)
        ...
        for set in self.dataset["framesets"]:
            ...
            for i in range(cfg.KEYPOINTDETECT.NUM_JOINTS):
                ...
                for cam in range(self.num_cameras):
                    if (
                        keypoints_l[cam][i][0] != 0
                        or keypoints_l[cam][i][1] != 0
                    ):
                        points2D[cam] = keypoints_l[cam][i][:2]
                        camsToUse.append(cam)
                keypoints3D_cam[i] = self.reproTools[
                    self.dataset["framesets"][set]["datasetName"]
                ].reconstructPoint(points2D.transpose(), camsToUse)
                if len(camsToUse) > 1:
            ...
            x_range = [
                np.min(keypoints3D_bb[:, 0]),
                np.max(keypoints3D_bb[:, 0]),
            ]
            y_range = [
                np.min(keypoints3D_bb[:, 1]),
                np.max(keypoints3D_bb[:, 1]),
            ]
            z_range = [
                np.min(keypoints3D_bb[:, 2]),
                np.max(keypoints3D_bb[:, 2]),
            ]
            tracking_area = np.array([x_range, y_range, z_range])
            x_cube_size_min = np.max(
                np.max(keypoints3D_bb[:, 0], axis=0)
                - np.min(keypoints3D_bb[:, 0], axis=0)
            )
            y_cube_size_min = np.max(
                np.max(keypoints3D_bb[:, 1], axis=0)
                - np.min(keypoints3D_bb[:, 1], axis=0)
            )
            z_cube_size_min = np.max(
                np.max(keypoints3D_bb[:, 2], axis=0)
    ...
    def _build_augpipe(self):
        ...
        if cfg.COLOR_MANIPULATION.ENABLED:
            ...
            augmentors.append(
                iaa.Sometimes(
                    cman.GAUSSIAN_BLUR.PROBABILITY,
            ...
            augmentors.append(
                iaa.Sometimes(
                    cman.LINEAR_CONTRAST.PROBABILITY,
                    iaa.LinearContrast(cman.LINEAR_CONTRAST.SCALE),
                )
            )
            augmentors.append(
                iaa.Sometimes(
                    cman.MULTIPLY.PROBABILITY,
                    iaa.Multiply(cman.MULTIPLY.SCALE),
                )
            )
            augmentors.append(
                iaa.Sometimes(
                    cman.PER_CHANNEL_MULTIPLY.PROBABILITY,
                    iaa.Multiply(
                        cman.PER_CHANNEL_MULTIPLY.SCALE,
    ...
    def __getitem__(self, idx):
        ...
        for frame_idx, img_id in enumerate(frameset_ids):
            ...
            if self.set_name == "train":
                translation_factors = np.random.uniform(-1.0, 1.0, 2)
        ...
        center3D = np.array(
            [
                int((max(x) + min(x)) / float(grid_spacing) / 2.0)
                * grid_spacing,
                int((max(y) + min(y)) / float(grid_spacing) / 2.0)
                * grid_spacing,
                int((max(z) + min(z)) / float(grid_spacing) / 2.0)
                * grid_spacing,
            ]
        )

        if self.set_name == "train":
            translation_margins = np.array(
                [
                    grid_size - (max(x) - min(x)),
            ...
            center3D += (
                np.array(
                    (translation_margins * translation_factors)
                    / float(grid_spacing)
                    / 2.0,
                    dtype=int,
                )
        ...
        keypoints3D_crop = (
            (keypoints3D + float(grid_size / 2.0) - center3D)
            / float(grid_spacing)
            / 2.0
        )

        heatmap_size = int(grid_size / grid_spacing / 2.0)
        heatmap3D = np.zeros(
            (
            ...
            if (
                ...
                heatmap3D[i, xx, yy, zz] = 255.0 * np.exp(
                    ...
                    * (
                        ...
                        + np.power(
                            (keypoints3D_crop[i][1] - yy) / (exponent), 2
                        )
                        + np.power(
                            (keypoints3D_crop[i][2] - zz) / (exponent), 2
                        )
                    )
                )
        sample = [
    ...
    def get_dataset_config(self, show_visualization=False):
        ...
        x_cube_size_min = np.percentile(
            tracking_areas[:, 0, 1] - tracking_areas[:, 0, 0], 95
        )
        y_cube_size_min = np.percentile(
            tracking_areas[:, 1, 1] - tracking_areas[:, 1, 0], 95
        )
        z_cube_size_min = np.percentile(
            tracking_areas[:, 2, 1] - tracking_areas[:, 2, 0], 95
        )
        min_cube_size = np.max(
            [x_cube_size_min, y_cube_size_min, z_cube_size_min]
        )
        ...
        # Cheap heuristic for GT Gaussian sigma: ~10% of the suggested
        # bbox in mm. Order-of-magnitude correct for "centered target with
        # ~2x motion range" scenarios, which is the typical case here.
        # Users should override based on knowledge of target size and
        # annotation noise — see the table in the GT_SIGMA_MM config doc.
        sigma_mm = max(1.0, 0.1 * final_bbox_suggestion)
...
class Normalizer(object):
    ...
    def __call__(self, sample):
        return [
            (sample[0].astype(np.float32) - self.mean) / self.std,
            sample[1],
            sample[2],
            sample[3],
            sample[4],
...
if __name__ == "__main__":
    ...
    project.load("calib_test")

    cfg = project.get_cfg()
    idx = 0
    training_set = Dataset3D(cfg=cfg, set="val")
    # , cameras_to_use = ['Camera_T', 'Camera_B', 'Camera_LBB'])
