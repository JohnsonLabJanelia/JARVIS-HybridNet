...
def predict3D(params):
    ...
    if not project.load(params.project_name):
        print(
            f"{CLIColors.FAIL}Could not load project: {project_name}! "
    ...
    caps, img_sizes = create_video_reader(params, reproTool, video_paths)

    if params.number_frames == -1:
        params.number_frames = (
            int(caps[0].get(cv2.CAP_PROP_FRAME_COUNT)) - params.frame_start
        )
    else:
        assert params.frame_start + params.number_frames <= caps[0].get(
    ...
    csvfile = open(
        os.path.join(params.output_dir, "data3D.csv"), "w", newline=""
    )
    writer = csv.writer(
    ...
    # imgs_orig = np.zeros((len(caps), img_size[1], img_size[0], 3)).astype(
    #     np.uint8
    # )
    imgs_orig = []
    for img_size in img_sizes:
        imgs_orig.append(
            np.zeros((img_size[1], img_size[0], 3)).astype(np.uint8)
        )

    for frame_num in tqdm(range(params.number_frames)):
        # load a batch of images from all cameras in parallel using joblib
        Parallel(n_jobs=12, require="sharedmem")(
            delayed(read_images)(cap, slice, imgs_orig)
            for slice, cap in enumerate(caps)
        )
        # imgs = (
        ...
        if params.progress_bar != None:
            params.progress_bar.progress(
                float(frame_num + 1) / float(params.number_frames)
            )
