...
class HybridNet:
    ...
    def __init__(
        ...
        if mode == "train":
            ...
            self.model_savepath = os.path.join(
                self.cfg.savePaths["HybridNet"], run_name
            )
            os.makedirs(self.model_savepath, exist_ok=True)

            self.logger = NetLogger(
                os.path.join(self.cfg.logPaths["HybridNet"], run_name)
            )
    ...
    def load_pose_pretrain(self, pose):
        ...
        weights_path = os.path.join(
            self.cfg.PARENT_DIR, "pretrained", pose, weights_name
        )
        if os.path.isfile(weights_path):
    ...
    def get_latest_weights(self):
        ...
        dirs = [
            os.path.join(search_path, d) for d in dirs
        ]  # add path to each file
        dirs.sort(key=lambda x: os.path.getmtime(x))
        dirs.reverse()
        for weights_dir in dirs:
            weigths_path = os.path.join(
                weights_dir,
    ...
    def train(
        ...
        for epoch in range(num_epochs):
            ...
            for counter, data in enumerate(progress_bar):
                ...
                for i, keypoints_batch in enumerate(keypoints):
                    for j, keypoint in enumerate(keypoints_batch):
                        if (
                            ...
                            acc += torch.sqrt(
                                torch.sum((keypoint - outputs[2][i][j]) ** 2)
                            )
                            count += 1
                acc = acc / count
                ...
                if streamlitWidgets != None:
                    streamlitWidgets[1].progress(
                        float(counter + 1) / float(len(training_generator))
                    )
            ...
            self.model.train()
            if streamlitWidgets != None:
                streamlitWidgets[0].progress(
                    float(epoch + 1) / float(num_epochs)
                )
                streamlitWidgets[2].markdown(f"Epoch {epoch+1}/{num_epochs}")
                streamlitWidgets[3].line_chart(
                    {"Train Loss": train_losses, "Val Loss": val_losses}
                )
                streamlitWidgets[4].line_chart(
                    {
                        "Train Accuracy [mm]": train_accs,
                        "Val Accuracy [mm]": val_accs,
                    }
                )
                st.session_state[
                    "HybridNet/" + self.training_mode + "/Train Loss"
                ] = train_losses
                st.session_state[
                    "HybridNet/" + self.training_mode + "/Val Loss"
                ] = val_losses
                st.session_state[
                    "HybridNet/" + self.training_mode + "/Train Accuracy"
                ] = train_accs
                st.session_state[
                    "HybridNet/" + self.training_mode + "/Val Accuracy"
                ] = val_accs
    ...
    def save_checkpoint(self, name):
        torch.save(
            self.model.state_dict(), os.path.join(self.model_savepath, name)
        )
